...
//	double scale = 1.30566e3;	
	...
	for(int i=0;i<N;i++){
		...
		for(int j=0;j<nEle;j++){
			...
			if(abs_e==0) continue;
			mean = scale*(p0*abs_e + p1*abs_e*abs_e+p2)/(1+p3*exp(-p4*abs_e));
			...
			PE[i] += gRandom->Gaus(mean,sigma)*abs_e/E_e;
		}
...
int main(int argc,char **argv){
	...
	//for(int j=367;j<371;j++){
		...
		//result[0] = a;
		//result[1] = b;
		//double p0 = gRandom->Gaus(par[0],par[0]*0.1);
		//double p1 = gRandom->Gaus(par[1],par[1]*0.1);
		//double p2 = gRandom->Gaus(par[2],par[2]*0.1);
		//double p3 = gRandom->Gaus(par[3],par[3]*0.1);
		//double p4 = gRandom->Gaus(par[4],par[4]*0.1);
		//for(int i=0;i<8;i++){
		//	data = get_gamma_sigma_mean(filename[i],a,b,p0,p1,p2,p3,p4,scale);
		//	mean = data[0];
		//	emean = data[1];
		//	result[i+2] = mean;
		//	result[i+10] = emean;
		...
		//
		//for(int i=2;i<9;i++){
        //    data = get_eplus_sigma_mean(i/1.0,a,b,p0,p1,p2,p3,p4,scale);
        //    mean = data[0];
        //    emean = data[1];
        //    result[i+16] = mean;
        //    result[i+23] = emean;
	...
	TFile *file = new TFile(Form("/home/zhangfy/gen_ml_data/data4_%i.root",seed),"recreate");
	t->Write();
	file->Close();
	return 0;
}
